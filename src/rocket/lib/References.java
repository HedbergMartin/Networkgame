package rocket.lib;

public class References {

	public static String CHANNEL_PLAYER_UPDATE = "pU";
	public static String CHANNEL_PLAYER_KICK = "pK";
	public static String CHANNEL_PLAYER_DC = "dC";
	public static String CHANNEL_OPENGUI = "oG";
	public static String CHANNEL_SPAWNPLAYER = "sP";

	public static final int GUI_MAINMENU = 0;
	public static final int GUI_JOINSCREEN = 1;
	public static final int GUI_LOBBY = 2;
	public static final int GUI_INGAME = 3;
	public static final int GUI_INGAMEMENU = 4;
}
